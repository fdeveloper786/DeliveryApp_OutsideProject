...
class Address extends StatefulWidget {
  ...
  Address(this.type, this.sub, this.saving, this.total,this.pincode, this.snapshot);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text('Shipping Address',style: GoogleFonts.poppins(color: Color(0xff00D3FF)),
        ),
      ),
      body: ListView(
        physics: BouncingScrollPhysics(),
        children: [
          Padding(padding: const EdgeInsets.only(left: 12, top: 12),
            child: Text('Choose your location',style:  GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 18)),
          ),
          SizedBox(height: 10),
          StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              if(snapshot.hasData&&snapshot.data.documents.length>0){
                delivery=double.parse(snapshot.data.documents[0]['delivery'].toString());
