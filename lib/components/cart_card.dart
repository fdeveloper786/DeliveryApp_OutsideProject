                        ...
                        children: [
                          Expanded( flex: 3,
                            child: Padding(padding: const EdgeInsets.only(left: 8),
                              child: Text('₹${snapshot.data['price']}', style: GoogleFonts.poppins(fontSize: 18,fontWeight: FontWeight.bold,color: Color(0xff00D3FF))),
                            ),
                          ),
                          Expanded(flex: 2,
                            ...
                            child: StreamBuilder<DocumentSnapshot>(
                                ...
                                builder: (context, snapshotData) {
                                  if(snapshotData.hasData){
                                    return snapshotData.data.data!=null?Row(
                                      children: [
                                        Expanded(flex: 4,
                                          child: IconButton(padding: EdgeInsets.all(0),icon: Icon(Icons.remove_circle,color: Color(0xff00D3FF),size: 35),onPressed: () async {
                                            ...
                                            try{
                                              if(snapshotData.data['count']>1){
                                                reference.document(snapshot.documentID).setData({
                                                  'count':snapshotData.data['count']-1,
                                                  'total':((snapshotData.data['count']-1)*snapshotData.data['price']),
                                                  'sellerTotal':((snapshotData.data['count']-1)*snapshotData.data['wholesale'])
                                                },merge: true);
                                              }else{
                                        ...
                                        Expanded(flex: 2,child: Text(snapshotData.data['count'].toString()??'',style: GoogleFonts.poppins(fontWeight: FontWeight.bold,color: Colors.black, fontSize: 16),
                                        textAlign: TextAlign.center,)),
                                        Expanded(flex: 4,
                                          child: IconButton(padding: EdgeInsets.all(0),icon: Icon(Icons.add_circle_outlined,color: Color(0xff00D3FF),size: 35),onPressed: () async {
                                            CollectionReference reference=Firestore.instance.collection('Users').document(UserData.user.uid).collection('Cart');
                                            try{
